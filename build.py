...
<div id="intro" class="wrap center">
  ...
  <h1>你是哪种依恋类型？</h1>
  <p class="sub">恋爱里你是哪一种？测测你的依恋风格，<br>看清自己的相处模式，把关系处得更舒服。</p>
  <button class="btn" onclick="start()">开始测试</button>
  <p class="tip">本测试为娱乐心理参考，结果不构成专业诊断。</p>
</div>
...
function start(){ document.getElementById('intro').style.display='none';
  document.getElementById('quiz').style.display='block'; renderQ(); }
...
function choose(i){ answers[cur]=i; renderQ(); }
function next(){
...
function finish(){
  ...
  const R=document.getElementById('result'); R.className='res-ct'; R.style.display='block';
  let h='';
